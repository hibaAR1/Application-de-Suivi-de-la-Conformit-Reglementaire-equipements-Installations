      ...
      <div className="content" style={{ maxWidth: 520 }}>
        <Plate style={{ padding: 24 }}>
          <div style={{ marginBottom: 18 }}>
            ...
            <div style={{ fontSize: 14, marginTop: 8 }}>{controle.reserve.nature}</div>
            <div className="spec-label" style={{ marginTop: 10 }}>Délai réglementaire</div>
            <div className="mono" style={{ fontWeight: 600 }}>{controle.reserve.delaiLevee}</div>
          </div>

          <form onSubmit={handleSubmit}>
            <div className="field">
              <label htmlFor="fichier">Justificatif de levée (PDF ou image)</label>
              <input id="fichier" type="file" accept=".pdf,image/*" onChange={(e) => setFichier(e.target.files?.[0] ?? null)} />
              {fichier && <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 4 }}>{fichier.name}</div>}
            </div>
            <div className="field">
              <label htmlFor="date-effective">Date de levée effective</label>
              <input id="date-effective" type="date" value={dateEffective} onChange={(e) => setDateEffective(e.target.value)} />
            </div>

            {erreur && <div style={{ color: 'var(--danger)', fontSize: 12.5, marginBottom: 12 }}>{erreur}</div>}

            <div style={{ display: 'flex', gap: 10 }}>
              <button type="submit" className="btn btn-primary">Confirmer la levée</button>
